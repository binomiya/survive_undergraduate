...
class HumanoidEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        motion=np.random.rand(120,14,3),
    ...
    def step(self, action):
        ...
        z_position_after = mass_z(self.model, self.sim)
        #print(z_position_after)
        #rewards = z_position_after
        ...
        #print(x_velocity)
        rewards =  - np.sum((self.sim.data.xipos - self.motion[t_flame])**2)
        #reward_velocitys = 
        #reward_rotations = 
        if t_flame==60:
            np.save("coordinate_humanoid.npy", self.sim.data.xipos)
            np.save("coordinate_mdm.npy", self.motion[t_flame])
